...
void GearsApplication::Update()
{
	...
	if (perspective)
	{
		...
		m_mainCamera.SetPerspectiveProjectionMatrix(static_cast<float>(std::numbers::pi) * 0.5f, aspectRatio, 0.1f, 10.f);
	}
	else {
		glm::vec3 halfSize(aspectRatio, 1.f, 10.f);
		m_mainCamera.SetOrthographicProjectionMatrix(-halfSize, halfSize);
	}

	glm::vec2 mousePosition = window.GetMousePosition(true);

	bool mouseIsTarget = false;
	if (mouseIsTarget)
	{
		m_mainCamera.SetViewMatrix(glm::vec3(0.f, 0.f, 1.f), glm::vec3(mousePosition, 0.f));
	}
	else {
		m_mainCamera.SetViewMatrix(glm::vec3(mousePosition, 1.f), glm::vec3(0.f, 0.f, 0.f));
	}
}
...
void GearsApplication::Render()
{
	...
	m_shaderProgram.SetUniform(m_viewProjMatrixUniform, m_mainCamera.GetViewProjectionMatrix());
	...
	glm::vec3 axisRight(0.f, 0.f, -1.f);

	// (todo) 03.2: Draw medium gear to the right
	float rightRatio = -16.f / 8.f;
	glm::mat4 rightGearMatrix(glm::translate(glm::mat4(1.0f), glm::vec3(0.75f, 0.f, 0.f)));
	rightGearMatrix = rightGearMatrix * glm::rotate(speed * time * rightRatio, axisRight);
	DrawGear(m_mediumGear, rightGearMatrix, Color(0.2f, 1.0f, 0.3f));

	// (todo) 03.3: Draw small gear at the top-left corner
	glm::vec3 axisSmall(0.f, 0.f, -1.f);
	float smallRatio = -16.0f / 30.f;
	glm::mat4 smallGearMatrix(glm::translate(glm::mat4(1.0f), glm::vec3(-1.f, 1.f, 0.f)));
	smallGearMatrix = smallGearMatrix * glm::rotate(speed * time * smallRatio + 0.050f, axisSmall);
	smallGearMatrix = glm::scale(smallGearMatrix, glm::vec3(7.5f));
	DrawGear(m_smallGear, smallGearMatrix, Color(0.3f, 0.2f, 1.0f));

	// (todo) 03.4: Draw small gear linked to the center gear
	glm::vec3 axisSmall2(0.f, 0.f, -1.f);
	float smallRatio2 = -16.0f / 30.f;
	glm::mat4 smallGearMatrix2(glm::translate(glm::mat4(1.0f), glm::vec3(0.f, 0.2f, 0.f)));
	smallGearMatrix2 = smallGearMatrix2 * glm::rotate(speed * time * smallRatio2 + 0.050f, axisSmall2);
	smallGearMatrix2 = centerGearMatrix * smallGearMatrix2;
	//smallGearMatrix2 = glm::scale(smallGearMatrix2, glm::vec3(7.5f));
	DrawGear(m_smallGear, smallGearMatrix2, Color(0.3f, 0.2f, 1.0f));
