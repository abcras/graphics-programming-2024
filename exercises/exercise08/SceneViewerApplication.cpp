...
void SceneViewerApplication::InitializeMaterial()
{
	...
	fragmentShaderPaths.push_back("shaders/ice_pbr.frag");
	Shader fragmentShader = ShaderLoader(Shader::FragmentShader).Load(fragmentShaderPaths);
	std::shared_ptr<ShaderProgram> iceShaderProgramPtr = std::make_shared<ShaderProgram>();
	iceShaderProgramPtr->Build(vertexShader, fragmentShader);
	...
	//}
	{
		...
		ShaderProgram::Location cameraPositionLocation = iceShaderProgramPtr->GetUniformLocation("CameraPosition");
		ShaderProgram::Location worldMatrixLocation = iceShaderProgramPtr->GetUniformLocation("WorldMatrix");
		ShaderProgram::Location viewProjMatrixLocation = iceShaderProgramPtr->GetUniformLocation("ViewProjMatrix");
		ShaderProgram::Location timeLocation = iceShaderProgramPtr->GetUniformLocation("Time");


		// Register shader with renderer
		m_renderer.RegisterShaderProgram(iceShaderProgramPtr,
			[=](const ShaderProgram& shaderProgram, const glm::mat4& worldMatrix, const Camera& camera, bool cameraChanged)
			{
				if (cameraChanged)
				{
			...
			m_renderer.GetDefaultUpdateLightsFunction(*iceShaderProgramPtr)
		);
	}
	...
	assert(iceShaderProgramPtr);

	//m_defaultMaterial = std::make_shared<Material>(shaderProgramPtr, filteredUniforms);
	m_iceMaterial = std::make_shared<Material>(iceShaderProgramPtr, filteredUniforms);
}
...
void SceneViewerApplication::InitializeModels()
{
	...
	m_iceMaterial->SetUniformValue("AmbientColor", glm::vec3(0.25f));
	m_iceMaterial->SetBlendEquation(Material::BlendEquation::Add, Material::BlendEquation::Add);
	m_iceMaterial->SetUniformValue("EnvironmentTexture", m_skyboxTexture);
	m_iceMaterial->SetUniformValue("EnvironmentMaxLod", maxLod);
	m_iceMaterial->SetUniformValue("Color", glm::vec3(0.0f, 0.4f, 0.0f));
	//m_iceMaterial->SetUniformValue("Time", Time);
	//m_iceMaterial->SetUniformValue()

	// Configure loader
	//ModelLoader loader(m_defaultMaterial);
	ModelLoader loader(m_iceMaterial);
...
void SceneViewerApplication::RenderGUI()
{
	...
	{
		if (ImGui::SliderFloat("Time", &Time, 0.f, 1000.0f))
		{
			m_iceMaterial->SetUniformValue("Time", Time);
		}
