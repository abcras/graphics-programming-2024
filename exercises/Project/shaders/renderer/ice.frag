...
void main()
{
	...
	if (dist < iceGrowth.y && dist > iceGrowth.x )
	{
		...
		vec3 temp = SampleNormalMap(VoronoiNoiseTexture, TexCoord * IceSamplingScale, normalize(WorldNormal), normalize(WorldTangent), normalize(WorldBitangent));

		data.normal = mix(data.normal, temp, ratio);

		//data.normal *= mix(vec3(1), temp, ratio);
	...
	if ( dist <= iceGrowth.x)
	{
		...
		data.normal = SampleNormalMap(VoronoiNoiseTexture, TexCoord * IceSamplingScale, normalize(WorldNormal), normalize(WorldTangent), normalize(WorldBitangent));
